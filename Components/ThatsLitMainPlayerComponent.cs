...
namespace ThatsLit
{
    ...
    public class ThatsLitMainPlayerComponent : MonoBehaviour
    {
            ...
            using (new ManagedStopWatch.RunningScope(ThatsLitPlugin.swGUI))
            {
                if (PlayerLitScoreProfile == null)
                {
                    if (Time.time - awakeAt < 30f && !ThatsLitPlugin.HideMapTip.Value)
                        GUILayout.Label("  [That's Lit] Brightness module is not supported on this map or disabled in configs.");
                ...
                Singleton<ThatsLitGameworld>.Instance.ScoreCalculator?.OnGUI(guiFrame < Time.frameCount);
        ...
        public TerrainDetailScore CalculateDetailScore(Vector3 enemyDirection, float dis, float verticalAxisAngle)
        {
            ...
            if (TerrainDetails?.Details5x5 == null) return cache; // Could be resizing? 
            ...
            foreach (var pos in it)
            {
                for (int i = 0; i < MAX_DETAIL_TYPES; i++)
                {
                    var info = TerrainDetails.Details5x5[pos * MAX_DETAIL_TYPES + i];
        ...
        public TerrainDetailScore CalculateCenterDetailScore(bool unscaled = false)
        {
            ...
            for (int i = 0; i < MAX_DETAIL_TYPES; i++)
            {
                var info = TerrainDetails.Details5x5[(2*5+2) * MAX_DETAIL_TYPES + i];
                if (!info.casted) continue;
                Utility.CalculateDetailScore(info.name, info.count, out var s1, out var s2);
                s1 *= scaling;
                s2 *= scaling;
